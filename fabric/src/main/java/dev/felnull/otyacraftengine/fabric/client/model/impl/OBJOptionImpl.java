...
public record OBJOptionImpl(boolean flipV, boolean ambientToFullbright) implements OBJOption {
    public static OBJOptionImpl of(Function<String, Optional<JsonElement>> elementFunction) {
        boolean flipV = elementFunction.apply("flip-v").map(e -> e.isJsonPrimitive() && e.getAsJsonPrimitive().isBoolean() && e.getAsBoolean()).orElse(false);
        boolean ambientToFullbright = elementFunction.apply("ambientToFullbright").map(e -> e.isJsonPrimitive() && e.getAsJsonPrimitive().isBoolean() && e.getAsBoolean()).orElse(true);
