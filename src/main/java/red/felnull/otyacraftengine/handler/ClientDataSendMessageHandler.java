...
public class ClientDataSendMessageHandler {
    ...
    public static void reversiveMessage(ClientDataSendMessage message, Supplier<NetworkEvent.Context> ctx) {
        if (message.frist) {
            ServerDataReceiver dr = new ServerDataReceiver(PlayerHelper.getUUID(ctx.get().getSender()), message.uuid, message.location, message.name, message.dataSize);
            dr.start();
        }
        ServerDataReceiver.addBufferBytes(PlayerHelper.getUUID(ctx.get().getSender()), message.uuid, message.data);
